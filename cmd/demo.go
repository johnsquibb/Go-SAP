...
func RunInteractiveBenchmark() {
	...
	for {
		if benchmark {
			fmt.Println("Press RETURN key to run benchmark, CTRL-C to quit.")
			fmt.Scanln()
		}
...
func tuneSystemClock() {
	...
	// ticksPerMicrosecond should represent the approximate amount of ticks that 1 MHz processor should delay.
	MhzDelay = uint64(ticksPerMicrosecond)
}
